...
**`docs/kds-knowledge/` 知識圖譜是本專案系統脈絡的唯一真相來源（single source of truth）。** 程式碼只是「現在長這樣」；圖譜才是「**為什麼這樣設計 / 邊界在哪 / 哪些是不可改的合約（★INVARIANT★）/ 驗證過沒**」——這些 code 讀不出來。
...
| 讀圖譜 / 寫筆記 / 巡檢 / 綁合約測試（★INVARIANT★→[test:]）/ 動 `docs/kds-knowledge/` | **`lumos-project-notes`** |
